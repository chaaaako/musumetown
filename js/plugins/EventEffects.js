...
/*:
 * @plugindesc [Ver 1.0.2] set various effects to events and player
 * @author Sasuke KANNAZUKI
 *
 * @help This plugin does not provide plugin commands.
 * 
 * Call follows from 'Move Route' -> 'Script'.
 * ■change tint
 ...
 */
/*:ja
 * @plugindesc [Ver 1.0.2]イベントやプレイヤーに様々な効果をセットします
 * @author 神無月サスケ
 *
...
(function() {
  ...
  //
  Game_Interpreter.prototype.currentInterpreter = function() {
  ...
  Game_Character.prototype.whichInterpreter = function () {
    return this._trigger === 4 ? this._interpreter :  $gameMap._interpreter;
  };

  Game_Character.prototype.currentInterpreter = function () {
    return this.whichInterpreter().currentInterpreter();
  };
  ...
  Game_CharacterBase.prototype.tint = function(tone, duration, needsWait) {
    ...
    if (needsWait) {
      this.currentInterpreter().wait(duration);
  ...
  // set ballon icon
  //
  Game_CharacterBase.prototype.balloon = function(num, wait) {
    this._balloonId = num;
    if (wait) {
      this.currentInterpreter().setWaitMode('balloon');
      this._waitCount = 76;
    }
    return true;
  };
